...
_ROTULOS_CAMPO = {
    # 2026-09-13, pedido direto da Thaís: texto de entrada/saída dividido em
    # 3 famílias pelo travessão "—" ("— fala — comentário") -- TEXE/TEFE e
    # TEDSA/TEFSA são a narração (antes/depois), FADEN/FADES é a fala em si
    # (do usuário na entrada, do Adam na saída). Pensamento de personagem
    # entre aspas continua dentro da narração normal, sem campo próprio.
    "TEXE": "Texto de entrada", "FADEN": "Fala de entrada", "TEFE": "Texto final de entrada",
    "RE": "Reação (RE)", "CE": "Contexto (CE)", "PIDE": "Pensamento (PIDE)",
    "TEDSA": "Texto de saída", "FADES": "Fala de saída", "TEFSA": "Texto final de saída",
    "RS": "Reação (RS)", "CS": "Contexto (CS)",
}
...
def _parear_campo(campo: str, palavras: List[str], marcadores: List[str]) -> str:
    ...
    junto (o PIDE guarda só os 3 primeiros tokens do pensamento; a partir de
    2026-09-13 a geração de marcador em si também só reserva esses 3 -- sem
    marcador fantasma sobrando entre o fim da entrada e o início da saída;
    o texto completo do pensamento continua preservado à parte, sem marcador)."""
    pares = " ".join(f"{p}({m})" for p, m in zip(palavras, marcadores))
    return pares if pares else "(vazio)"


def visao_bloco_memoria(bloco: dict, tokenizar, dividir_travessao) -> str:
    ...
    cada campo mostrado à parte -- nunca tudo misturado numa linha só como o
    inconsciente. `dividir_travessao` é injetado (mesma função de
    lovely_test.py) pra separar narração/fala sem duplicar a lógica aqui."""
    entrada = bloco.get("entrada", {})
    saida = bloco.get("saidas", [{}])[0]
    tokens_e = entrada.get("tokens", {})
    tokens_s = saida.get("tokens", {})

    texe_str, faden_str, tefe_str = dividir_travessao(entrada.get("texto", ""))
    linhas = [f"Bloco {bloco['bloco_id']}:", "  ENTRADA"]
    linhas.append(f"    {_ROTULOS_CAMPO['TEXE']}: {_parear_campo('TEXE', tokenizar(texe_str), tokens_e.get('TEXE', []))}")
    linhas.append(f"    {_ROTULOS_CAMPO['FADEN']}: {_parear_campo('FADEN', tokenizar(faden_str), tokens_e.get('FADEN', []))}")
    linhas.append(f"    {_ROTULOS_CAMPO['TEFE']}: {_parear_campo('TEFE', tokenizar(tefe_str), tokens_e.get('TEFE', []))}")
    reacao = entrada.get("reacao", "")
    linhas.append(f"    {_ROTULOS_CAMPO['RE']}: {_parear_campo('RE', [reacao] if reacao else [], tokens_e.get('RE', []))}")
    ...
    linhas.append(f"    {_ROTULOS_CAMPO['TEDSA']}: {_parear_campo('TEDSA', tokenizar(tedsa_str), tokens_s.get('TEDSA', []))}")
    linhas.append(f"    {_ROTULOS_CAMPO['FADES']}: {_parear_campo('FADES', tokenizar(fades_str), tokens_s.get('FADES', []))}")
    linhas.append(f"    {_ROTULOS_CAMPO['TEFSA']}: {_parear_campo('TEFSA', tokenizar(tefsa_str), tokens_s.get('TEFSA', []))}")
    saida_reacao = saida.get("reacao", "")
    linhas.append(f"    {_ROTULOS_CAMPO['RS']}: {_parear_campo('RS', [saida_reacao] if saida_reacao else [], tokens_s.get('RS', []))}")
    linhas.append(f"    {_ROTULOS_CAMPO['CS']}: {_parear_campo('CS', tokenizar(saida.get('contexto', '')), tokens_s.get('CS', []))}")

    return "\n".join(linhas)


def visao_universo_memoria(nome: str, dominio: str, blocos: List[dict], tokenizar, dividir_travessao) -> str:
    """A visão completa da MEMÓRIA (consciente) de um universo: título seguido
    de cada bloco, com a classificação em campos respeitada, sempre separados,
    nunca uma sopa só de palavra+marcador. `tokenizar`/`dividir_travessao` são
    injetados (de lovely_test.py) pra não duplicar lógica aqui."""
    titulo = f"IM = Universo {dominio} ({nome})"
    corpo = "\n\n".join(visao_bloco_memoria(b, tokenizar, dividir_travessao) for b in blocos)
...
_CORES_CAMPO = {
    "TEXE": "#2563eb", "TEFE": "#2563eb", "TEDSA": "#2563eb", "TEFSA": "#2563eb",  # texto/narração -- azul
    "FADEN": "#d97706", "FADES": "#d97706",  # fala (travessão) -- âmbar, cor própria por ser categoria nova
...
def visao_bloco_memoria_html(bloco: dict, tokenizar, dividir_travessao, bloco_inco: dict = None) -> str:
    ...
    cada palavra de Texto/Fala/Reação também mostra suas vars registradas
    (word-level, vindas do inconsciente -- Contexto e Pensamento NUNCA têm
    var, por definição, então não são cruzados aqui). Multivars (frase
    inteira) vêm direto do próprio bloco da memória, não do inconsciente.
    `dividir_travessao` é injetado (2026-09-13, pedido direto da Thaís):
    texto de entrada/saída sempre se divide em narração-antes / fala /
    narração-depois pelo travessão "—"; pensamento de personagem entre
    aspas continua dentro da narração normal, sem campo próprio."""
    entrada = bloco.get("entrada", {})
    saida = bloco.get("saidas", [{}])[0]
    tokens_e = entrada.get("tokens", {})
    tokens_s = saida.get("tokens", {})
    ...
    secoes = (
        ...
        + _campo_row_html(_ROTULOS_CAMPO["TEXE"], _CORES_CAMPO["TEXE"], tokenizar(texe_str), tokens_e.get("TEXE", []), vars_e)
        + _campo_row_html(_ROTULOS_CAMPO["FADEN"], _CORES_CAMPO["FADEN"], tokenizar(faden_str), tokens_e.get("FADEN", []), vars_e)
        + _campo_row_html(_ROTULOS_CAMPO["TEFE"], _CORES_CAMPO["TEFE"], tokenizar(tefe_str), tokens_e.get("TEFE", []), vars_e)
        + _multivars_row_html("Multivars (texto)", _CORES_CAMPO["TEXE"], entrada.get("Multivars_Texto_Entrada", []))
        + _campo_row_html("Reação (RE)", _CORES_CAMPO["RE"], [reacao] if reacao else [], tokens_e.get("RE", []), vars_e)
        ...
        + _campo_row_html(_ROTULOS_CAMPO["TEDSA"], _CORES_CAMPO["TEDSA"], tokenizar(tedsa_str), tokens_s.get("TEDSA", []), vars_s)
        + _campo_row_html(_ROTULOS_CAMPO["FADES"], _CORES_CAMPO["FADES"], tokenizar(fades_str), tokens_s.get("FADES", []), vars_s)
        + _campo_row_html(_ROTULOS_CAMPO["TEFSA"], _CORES_CAMPO["TEFSA"], tokenizar(tefsa_str), tokens_s.get("TEFSA", []), vars_s)
        + _multivars_row_html("Multivars (texto)", _CORES_CAMPO["TEDSA"], saida.get("Multivars_Texto_Saida", []))
        + _campo_row_html("Reação (RS)", _CORES_CAMPO["RS"], [saida_reacao] if saida_reacao else [], tokens_s.get("RS", []), vars_s)
...
def visao_universo_memoria_html(nome: str, dominio: str, blocos: List[dict], tokenizar, dividir_travessao, blocos_inco: List[dict] = None) -> str:
    inco_por_id = {b["Bloco_id"]: b for b in (blocos_inco or [])}
    corpo = "".join(
        visao_bloco_memoria_html(b, tokenizar, dividir_travessao, inco_por_id.get(str(b["bloco_id"])))
        for b in blocos
    )
