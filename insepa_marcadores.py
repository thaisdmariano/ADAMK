...
def tokenizar(texto: str) -> List[str]:
    ...
    Isso NÃO é usado pra reação/emoção -- reação nunca é tokenizada palavra a
    palavra (uma carinha como ":3" é UM símbolo, não uma sequência de
    pontuação solta). Isso é resolvido tratando o campo de reação como
    atômico onde ele é lido, não aqui no tokenizador geral -- reconhecer
    carinha por conteúdo (lista fixa ou padrão) seria voltar a "adivinhar
    forma", que é exatamente o que o INSEPA não faz.
    """
